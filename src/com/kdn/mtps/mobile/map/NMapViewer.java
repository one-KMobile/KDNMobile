...
//		item.setRightAccessory(true, NMapPOIflagType.CLICKABLE_ARROW);
		item.setHeadText("headerrrrrrr");
		item.setTailText("setTailText");
		item.setSnippet("setSnippet");
		item.setRightButton(true);
		item.setRightAccessory(true, NMapPOIflagType.CUSTOM_BASE);
		poiData.addPOIitem(127.061, 37.51, "Pizza 123-456", markerId, 0);
		poiData.addPOIitem(127.06269776960362, 37.498592797284864, "Pizza zzzzzz", markerId, 0);
		poiData.endPOIdata();
		...
		public View onCreateCalloutOverlayView(NMapOverlay itemOverlay, NMapOverlayItem overlayItem, Rect itemBounds) {
			...
			if (overlayItem != null) {
				...
				if (title != null && title.length() > 5) {
					return new NMapCalloutCustomOverlayView(NMapViewer.this, itemOverlay, overlayItem, itemBounds, 0);
				}
			}
