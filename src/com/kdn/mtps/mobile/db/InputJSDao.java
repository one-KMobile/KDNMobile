...
public class InputJSDao extends BaseDao{
	static InputJSDao inputJSDao;

	private InputJSDao(Context ctx) {
		tableName = "INPUT_JS";
		this.ctx = ctx;
	}

	public static InputJSDao getInstance(Context ctx) {
		if (inputJSDao == null)
			inputJSDao = new InputJSDao(ctx);
		return inputJSDao;
	}
	
	public void Append(JSInfo row) {
		Append(row, -1);
	}
	
	public void Append(JSInfo row, int idx) {
		SQLiteDatabase db = DBHelper.getInstance(ctx);
		ContentValues updateRow = new ContentValues();
		
		if (idx != -1) {
			updateRow.put(JSInfo.COLS.IDX, idx);
		}
		
		updateRow.put(JSInfo.COLS.MASTER_IDX, row.master_idx);
		updateRow.put(JSInfo.COLS.WEATHER, row.weather);
		updateRow.put(JSInfo.COLS.AREA_TEMP, row.area_temp);
		updateRow.put(JSInfo.COLS.CIRCUIT_NO, row.circuit_no);
		updateRow.put(JSInfo.COLS.CIRCUIT_NAME, row.circuit_name);
		updateRow.put(JSInfo.COLS.CURRENT_LOAD, row.current_load);
		updateRow.put(JSInfo.COLS.CONDUCTOR_CNT, row.conductor_cnt);
		updateRow.put(JSInfo.COLS.LOCATION, row.location);
		updateRow.put(JSInfo.COLS.C1_JS, row.c1_js);
		updateRow.put(JSInfo.COLS.C1_JSJ, row.c1_jsj);
		updateRow.put(JSInfo.COLS.C1_YB_RESULT, row.c1_yb_result);
		updateRow.put(JSInfo.COLS.C1_POWER_NO, row.c1_power_no);
		updateRow.put(JSInfo.COLS.C2_JS, row.c2_js);
		updateRow.put(JSInfo.COLS.C2_JSJ, row.c2_jsj);
		updateRow.put(JSInfo.COLS.C2_YB_RESULT, row.c2_yb_result);
		updateRow.put(JSInfo.COLS.C2_POWER_NO, row.c2_power_no);
		updateRow.put(JSInfo.COLS.C3_JS, row.c3_js);
		updateRow.put(JSInfo.COLS.C3_JSJ, row.c3_jsj);
		updateRow.put(JSInfo.COLS.C3_YB_RESULT, row.c3_yb_result);
		updateRow.put(JSInfo.COLS.C3_POWER_NO, row.c3_power_no);
...
//		dbCheck();
	...
	public ArrayList<JSInfo> selectJS(String mIdx) {
		
		ArrayList<JSInfo> jsList = new ArrayList<JSInfo>();
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				String master_idx = cursor.getString(cursor.getColumnIndex(JSInfo.COLS.MASTER_IDX));
