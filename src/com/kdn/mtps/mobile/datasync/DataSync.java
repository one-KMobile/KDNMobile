...
public class DataSync {
    ...
    public final int TAG_CALL_API_JGINFO_SUBLIST = TAG_DATASYNC_DEFAULT + 6;
    public final int TAG_CALL_API_GHINFO_SUBLIST = TAG_DATASYNC_DEFAULT + 7;
    public final int TAG_CALL_API_PRINFO_SUBLIST = TAG_DATASYNC_DEFAULT + 8;
    public final int TAG_CALL_API_MHINFO_SUBLIST = TAG_DATASYNC_DEFAULT + 9;
    ...
    JGPSubInfoList jgpSubInfoList;
    GHSubInfoList ghSubInfoList;
    PRPSubInfoList prpSubInfoList;
    PRKSubInfoList prkSubInfoList;
    MHISubInfoList mhiSubInfoList;
    MHBSubInfoList mhbSubInfoList;
    MHGSubInfoList mhgSubInfoList;
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
			...
			case TAG_CALL_API_HKINFO_SUBLIST: //
                ...
                case TAG_CALL_API_TOWER_LIST: // 8
                    pd.setProgress((int) ((7 / (float) 7) * 100));
                    getTowerList();
                    break;
                case TAG_CALL_API_JGINFO_SUBLIST: // 4
                    pd.setProgress((int) ((3 / (float) 7) * 100));
                    getJGInfoSubList();
                    break;
                case TAG_CALL_API_GHINFO_SUBLIST: // 5
                    ...
                    getGHInfoSubList();
                    break;
                case TAG_CALL_API_PRINFO_SUBLIST: // 6
                    pd.setProgress((int) ((5 / (float) 7) * 100));
                    getPRInfoSubList();
                    break;
                case TAG_CALL_API_MHINFO_SUBLIST: // 7
...
//						ToastUtil.show(ctx, "데이터 동기화가 실패하였습니다.");
    ...
    public void apiTracksList() {
        ATask.executeVoid(new ATask.OnTask() {
            ...
            public void onPost() {
                ...
                if (isComplete) {
                    mHandler.sendEmptyMessage(TAG_CALL_API_JGINFO_SUBLIST);
    ...
    public void getJGInfoSubList() {
        ...
        ATask.executeVoidPublishProgress(new ATask.OnTaskPublishProgress() {
            ...
            public void onBG(OnPublishProgress onPublish) {
                ...
                jgpSubInfoList = ApiManager.jgpInfoSubList();
                ...
                if (jgpSubInfoList != null && jgpSubInfoList.result != null && ApiManager.RESULT_OK.equals(jgpSubInfoList.code)) {

                    SQLiteDatabase db = DBHelper.getInstance(ctx);
                    try {
                        db.beginTransaction();
                        InputJGPSubInfoDao.getInstance(ctx).DeleteAll();

                        List<JGPSubInfo> subList = jgpSubInfoList.result;
                        for (int i = 0; i < subList.size(); i++) {
                            JGPSubInfo subInfo = subList.get(i);
                            InputJGPSubInfoDao inputJGPSubInfoDao = InputJGPSubInfoDao.getInstance(ctx);
                            inputJGPSubInfoDao.Append(subInfo);
                        }
    ...
    public void getGHInfoSubList() {
        ...
        ATask.executeVoidPublishProgress(new ATask.OnTaskPublishProgress() {
            ...
            public void onPost() {
                ...
                if (isComplete) {
                    //mHandler.sendEmptyMessage(TAG_CALL_API_BRINFO_SUBLIST);
                    mHandler.sendEmptyMessage(TAG_CALL_API_PRINFO_SUBLIST);
                } else {
                    mHandler.sendEmptyMessage(TAG_END_SYNC);
                }
            }
    ...
    public void getPRInfoSubList() {
        ...
        ATask.executeVoidPublishProgress(new ATask.OnTaskPublishProgress() {
            ...
            @Override
            public void onBG(OnPublishProgress onPublish) {
                prpSubInfoList = ApiManager.prpInfoSubList();
                prkSubInfoList = ApiManager.prkInfoSubList();

                isComplete = false;

                if (prpSubInfoList != null && prpSubInfoList.result != null && ApiManager.RESULT_OK.equals(prpSubInfoList.code)) {

                    SQLiteDatabase db = DBHelper.getInstance(ctx);
                    try {
                        db.beginTransaction();
                        InputPRPSubInfoDao.getInstance(ctx).DeleteAll();

                        List<PRPSubInfo> subList = prpSubInfoList.result;
                        for (int i = 0; i < subList.size(); i++) {
                            PRPSubInfo subInfo = subList.get(i);
                            InputPRPSubInfoDao inputPRPSubInfoDao = InputPRPSubInfoDao.getInstance(ctx);
                            inputPRPSubInfoDao.Append(subInfo);
                        }
                ...
                if (prkSubInfoList != null && prkSubInfoList.result != null && ApiManager.RESULT_OK.equals(prkSubInfoList.code)) {

                    SQLiteDatabase db = DBHelper.getInstance(ctx);
                    try {
                        db.beginTransaction();
                        InputPRKSubInfoDao.getInstance(ctx).DeleteAll();

                        List<PRKSubInfo> subList = prkSubInfoList.result;
                        for (int i = 0; i < subList.size(); i++) {
                            PRKSubInfo subInfo = subList.get(i);
                            InputPRKSubInfoDao inputPRKSubInfoDao = InputPRKSubInfoDao.getInstance(ctx);
                            inputPRKSubInfoDao.Append(subInfo);
                        }
            ...
            public void onPost() {
                ...
                if (isComplete) {
                    //mHandler.sendEmptyMessage(TAG_CALL_API_BRINFO_SUBLIST);
                    mHandler.sendEmptyMessage(TAG_CALL_API_MHINFO_SUBLIST);
                } else {
                    mHandler.sendEmptyMessage(TAG_END_SYNC);
                }
            }
    ...
    public void getMHInfoSubList() {
        ...
        ATask.executeVoidPublishProgress(new ATask.OnTaskPublishProgress() {
            ...
            @Override
            public void onBG(OnPublishProgress onPublish) {
                mhiSubInfoList = ApiManager.mhiInfoSubList();
                mhbSubInfoList = ApiManager.mhbInfoSubList();
                mhgSubInfoList = ApiManager.mhgInfoSubList();

                isComplete = false;

                if (mhiSubInfoList != null && mhiSubInfoList.result != null && ApiManager.RESULT_OK.equals(mhiSubInfoList.code)) {

                    SQLiteDatabase db = DBHelper.getInstance(ctx);
                    try {
                        db.beginTransaction();
                        InputMHISubInfoDao.getInstance(ctx).DeleteAll();

                        List<MHISubInfo> subList = mhiSubInfoList.result;
                        for (int i = 0; i < subList.size(); i++) {
                            MHISubInfo subInfo = subList.get(i);
                            InputMHISubInfoDao inputMHISubInfoDao = InputMHISubInfoDao.getInstance(ctx);
                            inputMHISubInfoDao.Append(subInfo);
                        }
                ...
                if (mhbSubInfoList != null && mhbSubInfoList.result != null && ApiManager.RESULT_OK.equals(mhbSubInfoList.code)) {

                    SQLiteDatabase db = DBHelper.getInstance(ctx);
                    try {
                        db.beginTransaction();
                        InputMHBSubInfoDao.getInstance(ctx).DeleteAll();

                        List<MHBSubInfo> subList = mhbSubInfoList.result;
                        for (int i = 0; i < subList.size(); i++) {
                            MHBSubInfo subInfo = subList.get(i);
                            InputMHBSubInfoDao inputMHBSubInfoDao = InputMHBSubInfoDao.getInstance(ctx);
                            inputMHBSubInfoDao.Append(subInfo);
                        }
                ...
                if (mhgSubInfoList != null && mhgSubInfoList.result != null && ApiManager.RESULT_OK.equals(mhgSubInfoList.code)) {

                    SQLiteDatabase db = DBHelper.getInstance(ctx);
                    try {
                        db.beginTransaction();
                        InputMHGSubInfoDao.getInstance(ctx).DeleteAll();

                        List<MHGSubInfo> subList = mhgSubInfoList.result;
                        for (int i = 0; i < subList.size(); i++) {
                            MHGSubInfo subInfo = subList.get(i);
                            InputMHGSubInfoDao inputMHGSubInfoDao = InputMHGSubInfoDao.getInstance(ctx);
                            inputMHGSubInfoDao.Append(subInfo);
                        }
